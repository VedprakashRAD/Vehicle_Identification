...
class WorkingVehicleTracker:
    ...
    def _run_vehicle_detection(self, frame: np.ndarray) -> List:
        """Run YOLO inference on frame for vehicle detection"""
        ...
        for result in results:
            if result.boxes is not None:
                for box in result.boxes:
                    ...
                    # Only process vehicle classes (car, motorcycle, bus, truck)
    ...
    def _process_detections(self, detections: List, processed_frame: np.ndarray, original_frame: np.ndarray) -> Dict:
        """Process vehicle detections and handle license plates with special handling for cars"""
        current_detections = {'car': 0, 'motorcycle': 0, 'bus': 0, 'truck': 0}
        ...
        for detection in detections:
            ...
            # Special handling: For cars, proceed with license plate recognition
            # For other vehicle types, we can optionally skip plate recognition or do it with lower priority
            if vehicle_type == 'car':
                # Process license plate for cars (priority handling)
                self._process_license_plate(original_frame, bbox, vehicle_id, vehicle_type, processed_frame)
            else:
                # For other vehicle types, still process plates but with optional handling
    ...
    def _track_vehicle(self, bbox: Tuple, vehicle_type: str) -> str:
        ...
        for vid, data in self.tracked_vehicles.items():
            ...
            if abs(center_x - old_x) < threshold and abs(center_y - old_y) < threshold:
                ...
                # Update vehicle type if not set or if it's more specific
                if not data['type'] or data['type'] == 'unknown':
                    self.tracked_vehicles[vid]['type'] = vehicle_type
                return vid
